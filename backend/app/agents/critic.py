...
class CriticAgent:
    ...
    async def evaluate(
        ...
        """Attach freshness/resonance/feasibility scores to each node dict."""
        ideas_payload = [{"title": n["title"], "description": n.get("description"), "tags": n.get("tags", [])} for n in nodes]
        ...
        for node in nodes:
            s = score_map.get(node["title"], {})
            node["score_freshness"] = s.get("freshness")
            node["score_resonance"] = s.get("resonance")
            node["score_feasibility"] = s.get("feasibility")
