...
- 自建轻量 Agent 编排 — 不使用 LangChain，直接封装 LLM API 调用
- openai Python SDK — LLM 调用，兼容 OpenAI / Moonshot / DeepSeek 等任意 OpenAI-compatible 接口
- WebSocket (FastAPI) — 实时推送进化结果
- In-Memory Store (`store.py`) — 轻量会话状态管理，无需数据库
...
### In-Memory Store 结构（`app/store.py`）

后端使用纯内存 dict 存储，无需数据库。进程重启后数据清空（MVP 阶段可接受）。

```python
sessions: dict[str, dict]   # session_id -> session dict
nodes:    dict[str, dict]   # node_id    -> node dict
briefs:   dict[str, dict]   # session_id -> brief dict
```

每个 dict 的字段与下方 TypeScript 类型一一对应（snake_case）。
...
def make_client(api_key: str, base_url: str | None = None) -> AsyncOpenAI:
    """创建 OpenAI-compatible 客户端。留空 base_url 使用 OpenAI 官方；
    设置 base_url 可切换到 Moonshot、DeepSeek 等兼容接口。"""
...
async def call_llm(client: AsyncOpenAI, model: str, system: str, user: str) -> dict | list:
    """统一的 LLM 调用封装，返回解析后的 JSON。JSON 解析失败时重试一次。"""
...
2. 初始化 FastAPI 后端项目
3. 实现 In-Memory Store（`store.py`）：sessions / nodes / briefs dict + 辅助函数
4. 编写 TypeScript 类型定义文件
5. 配置环境变量：`OPENAI_API_KEY`（以及可选的 `LLM_BASE_URL`、`LLM_MODEL`）
...
- 模型：任意 OpenAI-compatible 接口，通过 `LLM_BASE_URL` + `LLM_MODEL` 配置
- 所有 Agent prompt 要求返回纯 JSON，不带 markdown 标记
- 单次进化的 token 预算：input ~2000, output ~1000
- 完整流程（3 轮 + brief）约 8000-10000 tokens
- 错误处理：JSON 解析失败时重试一次，仍失败则返回预设的 fallback 变异（在 `llm/client.py` 中处理）
...
LLM_BASE_URL=             # 留空使用 OpenAI；填入可切换 Moonshot、DeepSeek 等
LLM_MODEL=gpt-4o-mini     # 所选 provider 的模型名
```

Provider 示例：
- **OpenAI**: `LLM_BASE_URL=`（空），`LLM_MODEL=gpt-4o`
...
5. **JSON 解析要健壮。** LLM 输出不总是完美的 JSON，要有清理和重试逻辑（`llm/client.py` 已处理）。
6. **限制进化轮数。** MVP 最多 5 轮进化（约 20 个节点），避免性能问题。
7. **In-Memory Store 是有意为之。** 不需要 PostgreSQL 或 Redis，进程重启数据清空在 MVP 阶段可接受。如需持久化，可在 `store.py` 中替换为文件/数据库后端，接口不变。
8. **LLM provider 可热切换。** 只需修改 `.env` 中的 `LLM_BASE_URL` 和 `LLM_MODEL`，无需改代码。
